...
def _build_config(env: EnvConfig) -> dict[str, str]:
    return {
        ...
        "conform_path": f"{env.silver_base_path}/{env.catalog}/{env.schemas['silver']}/s_erp_plan_catalog/s_conform_erp_plan_catalog",
    }
...
def run_silver_erp_plan_catalog(spark: SparkSession, env: EnvConfig) -> None:
    ...
    try:
        ...
        logger.info("Silver erp_plan_catalog start | run_id=%s", run_id)
        ...
        logger.info(
            "Silver erp_plan_catalog SUCCESS | rows_in=%d | rows_out=%d | rows_quarantined=%d",
            rows_in,
            rows_out,
            rows_quarantined,
        )
