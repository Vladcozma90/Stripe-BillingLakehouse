...
def _build_stage_stripe_customers(incr_df: DataFrame, run_id: str) -> DataFrame:
    df = (
        ...
        .withColumn("email", lower(trim(col("data.email"))).cast("string"))
...
def run_silver_stripe_customers(spark: SparkSession, env: EnvConfig) -> None:
    ...
    try:
        ...
        if rows_out == 0:
            ...
            logger.info(
                "No valid stripe customer rows to merge | run_id=%s",
                run_id,
            )
            return
        ...
        logger.info(
            "Silver stripe_customers SUCCESS | rows_in=%d | rows_out=%d | rows_quarantined=%d",
            rows_in,
            rows_out,
            rows_quarantined
        )
        ...
        logger.exception("Silver stripe_customers FAILED | run_id=%s", run_id)
        raise
