...
def _build_config(env: EnvConfig) -> dict[str, str]:
    return {
        ...
        "gold_table": f"{env.catalog}.{env.schemas['gold']}.g_dim_account_master",
        ...
        "gold_path": f"{env.gold_base_path}/{env.catalog}/{env.schemas['gold']}/g_dim_account_master",
    }
...
def bootstrap_erp_account_master_snapshot(spark: SparkSession, env: EnvConfig) -> None:
    ...
    logger.info("Creating/validating erp_account_master_snapshot in schema %s", f"{env.catalog}.{env.schemas['gold']}")
