...
This document describes the current testing setup for the project.

The project currently has two validation layers:

1. Unit tests for Python services, config loading, and job imports.
2. Smoke tests for Airflow → Databricks orchestration.
...
This file tests the DQ helper functions in:
...
* invalid dates and timestamps are handled as DQ failures instead of crashing the Spark job;
* generated DQ failure messages include useful rule information.

One issue found during testing was that malformed date/timestamp values caused Spark `CAST_INVALID_INPUT` errors. The DQ logic was updated to use `try_cast`, so invalid date/timestamp values are now counted as failed DQ checks.

### `test_envs.py`

This file checks that the project configuration can be loaded correctly.

It validates the expected structure of the config file, including paths, schemas, and dataset-level settings.
...
The tests are run inside Docker because this is closer to the actual local runtime used by Airflow. It also verifies that Java/PySpark, project imports, and Airflow-related dependencies are available in the container.
...
Before committing screenshots, sensitive values should be hidden where needed, especially tokens, user emails, workspace IDs, and workspace URLs.
...
Those checks will be part of the integration testing stage once the real Databricks jobs are wired.

---

## Next testing steps

The next step is to validate the real Databricks jobs in order:
...
After those jobs are created and tested individually, they can be orchestrated from Airflow as a full pipeline.